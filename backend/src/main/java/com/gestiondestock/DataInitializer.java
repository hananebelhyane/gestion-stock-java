...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        // Seed Products
        Produit p1 = new Produit(); p1.setNom("Paper A4"); p1.setDescription("500 sheets pack"); p1.setPrixUnitaire(3.5); em.persist(p1);
        Produit p2 = new Produit(); p2.setNom("Ink Cartridge"); p2.setDescription("Black"); p2.setPrixUnitaire(19.0); em.persist(p2);
        Produit p3 = new Produit(); p3.setNom("USB Cable"); p3.setDescription("Type-C"); p3.setPrixUnitaire(2.5); em.persist(p3);
        Produit p4 = new Produit(); p4.setNom("Laptop Stand"); p4.setDescription("Aluminum"); p4.setPrixUnitaire(18.0); em.persist(p4);

        // Seed Stock (one row per product)
        Stock s1 = new Stock(); s1.setProduit(p1); s1.setQuantiteDisponible(120); s1.setSeuilAlerte(20); em.persist(s1);
        Stock s2 = new Stock(); s2.setProduit(p2); s2.setQuantiteDisponible(5);   s2.setSeuilAlerte(10); em.persist(s2);
        Stock s3 = new Stock(); s3.setProduit(p3); s3.setQuantiteDisponible(0);   s3.setSeuilAlerte(5);  em.persist(s3); // out-of-stock
        Stock s4 = new Stock(); s4.setProduit(p4); s4.setQuantiteDisponible(40);  s4.setSeuilAlerte(10); em.persist(s4);
