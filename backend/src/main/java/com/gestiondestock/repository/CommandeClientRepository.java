...
public interface CommandeClientRepository extends JpaRepository<CommandeClient, UUID> {
    ...
    // « Compte le nombre de lignes dans la table CommandeClient dont la colonne
    // statut a la valeur donnée. »
    ...
    // Trouver les commandes en attente d'un client
    List<CommandeClient> findByClientIdAndStatut(UUID clientId, CommandeClient.StatutCommande statut);

    // Toutes les commandes d'un client
    List<CommandeClient> findByClientId(UUID clientId);

    // Trouver la dernière commande en attente d'un client (méthode dérivée Spring
    // Data)
    Optional<CommandeClient> findFirstByClientIdAndStatutOrderByDateCommandeDesc(UUID clientId,
            CommandeClient.StatutCommande statut);
}
